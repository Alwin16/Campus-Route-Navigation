...
public:
  ...
  vector<tuple<int, int, string>> adjacentEdges; // {destination vertex, weight, direction}
...
public:
  ...
  void addEdge(int src, int dest, int weight, const string &directionSrcToDest,
    ...
    vertices[dest].adjacentEdges.push_back(
        {src, weight, directionDestToSrc}); // For undirected graph
  }
...
public:
  static tuple<vector<int>, vector<int>, int> shortestPath(Graph &graph,
    ...
    vector<int> predecessors(graph.vertices.size(), -1); // To track the path
    ...
    while (!pq.empty()) {
      ...
      for (const auto &neighbor : graph.vertices[currIndex].adjacentEdges) {
        ...
        if (distances[nextIndex] > distances[currIndex] + weight) {
          ...
          predecessors[nextIndex] = currIndex; // Update predecessor
...
int main() {
  ...
  while (true) {
    ...
    // Print the shortest path with detailed directions
